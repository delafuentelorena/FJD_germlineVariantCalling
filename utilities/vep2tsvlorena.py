    ...
    with open(args.input_file) as in_f:
        ...
        for line in in_f:
            ...
            if (
                ...
                ## adding genotype type for children if GT and children specified.
                if out_header!='':
                    print("hola")
                    print(out_header)
                    print("adios")
                    out_HEADER = add_new_columns(args, out_header, wrote_header) # change header
                    print(out_HEADER)
                    if args.output_file:
                        out_f.write('%s\n' % out_HEADER)
                    else:
                ...
                if out_str:
                    out_STR = add_new_columns(args, out_HEADER, wrote_header, out_str) # change line
                    if args.output_file:
                        out_f.write('%s\n' % out_STR)
                    else:
                        print out_STR                   
...
def add_new_columns(args, str_header, wrote_header, str_line=None):
    ...
    individuals = []
    list_STR = []
    # adding HOM_VAR/HOM_REF/HET/

    if wrote_header:


        # add raquel frequencies
        ...
        if args.internal_freq and "GT" in args.genotype_fields:
            ...
            for m in list_str:
                ...
                list_STR.append("\t".join(list_transc+[str(freq.count("HOM_REF")),str(freq.count("HOM_VAR")),str(freq.count("HET"))]))
                print(list_STR)
            return "\n".join(list_STR)
...
def parse_args():
    ...
    parser.add_argument(
        '-i', '--internal_freq', action='store_true',
